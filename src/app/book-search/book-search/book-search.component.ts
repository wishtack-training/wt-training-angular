...
export class BookSearchComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    const controls = Object.entries(this.searchForm.controls).map(([name, control]) => ({
      name,
      control
    }));
    ...
    const cumulatedChanges$ = changes$
      .pipe(
        scan((acc, change) => {
          return {
            ...acc,
    ...
    const searchCriteria$ =
      merge(this._searchTrigger$, interval(1000))
        .pipe(
          map(() => this.searchForm.value),
          distinctUntilChanged()
        );

    this.books$ = searchCriteria$
      .pipe(
    ...
    this._searchTrigger$
      .pipe(
        withLatestFrom(cumulatedChanges$)
      )
